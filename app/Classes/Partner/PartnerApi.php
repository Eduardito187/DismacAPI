...
class PartnerApi{
    ...
    public function generateAnalyticsReportYear(string $type, string $code){
        ...
        $sumValuesByMonth = Analytics::where($this->text->getType(), $type)->where($this->text->getCode(), $code)
            ->whereBetween($this->text->getCreated(), [$firstDayOfYear, $lastDayOfYear])
    ...
    public function generateAnalyticsReportMonths(string $type, string $code){
        ...
        foreach ($sumByWeek as $weekNumber => $total) {
            $weekName = $this->text->getWeek().$weekNumber;
    ...
    public function countWarehousesPartner(int $id_partner){
        return Warehouse::where($this->text->getStatus(), 1)->sum($this->text->getStatus());
