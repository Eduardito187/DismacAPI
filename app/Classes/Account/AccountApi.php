...
class AccountApi{
    ...
    public function searchAccount(Request $request){
        ...
        $Accounts = Account::select("id","name","email")->with(['accountLogin' => function ($query) {
            $query->select('status');
        }])->whereIn("id", $AccountPartner)->where("name", "like", "%".$param."%")->
        orwhere("email", "like", "%".$param."%")->whereIn("id", $AccountPartner)->get()->toArray();
        return $Accounts;
    }
