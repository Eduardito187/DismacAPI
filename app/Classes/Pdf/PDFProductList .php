...
class PDFProductList extends TCPDF{
    ...
    private function writeProducts(array $products){
        foreach ($products as $product) {
            $this->Cell(0, 10, $product[Text::COLUMN_NAME], 0, 1);
            if (!is_null($product[Text::BRAND])){
                $this->Cell(0, 10, $product[Text::BRAND], 0, 1);
            }

            // Si el precio especial es mayor a 0, tacha el precio.
            if ($product[Text::SPECIAL_PRICE] > 0) {
                $this->SetFont('', 'S');
                $this->Cell(0, 10, 'Bs '.$product[Text::PRICE], 0, 1, 'T');
                $this->SetFont('', '');
                $this->Cell(0, 10, 'Bs '.$product[Text::SPECIAL_PRICE], 0, 1);
            } else {
                $this->Cell(0, 10, 'Bs '.$product[Text::PRICE], 0, 1);
            }
            $this->Ln(5);
            $imageWidth = 50;
            $imageHeight = 50;
